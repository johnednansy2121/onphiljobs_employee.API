...
module.exports = function(){
    const baseRoute = basePath + 'employer'

    this.post(baseRoute + '/signup-employer', ValidatorRequest.validateRequest(signUpSchema), (req, res) => {
        return new Promise((resolve, reject) => {
            console.log("accepted",req.body)
            const tag  = req.query.tag ? req.query.tag : ''
            EmployerService.signup({ ...req.body, tag })
                .then(data => resolve(
                    res.json(Response.Success.Custom('successfully signed up employer.', data
                    ))))
                .catch(error => {console.log("failed");reject(Response.Error.Custom(res, error.message))})
        })
    })

    this.post(baseRoute + '/login-employer', ValidatorRequest.validateRequest(employerLoginSchema), (req, res) => {
        return new Promise((resolve, reject) => {
            EmployerService.login(req.body)
                .then(data => resolve(res.json(Response.Success.Custom('successfully logged in employer.', { ...data }))))
                .catch(error => { reject(Response.Error.Custom(res, error.message)) })
        })
    })

    this.get(baseRoute + '/verify-employer/:id', (req, res) => {
        return new Promise((resolve, reject) => {
            const code = req.params.id
            EmployerService.verify(code)
    ...
    this.post(baseRoute + '/2FA-employer', async(req, res) => {
        try {
    ...
    this.get(baseRoute + '/2FA/employer-settings', authorization('employer-2FA'), async(req, res) => {
        try {
            const result = await UserService.get2FASettings(req.user)

            res.status(200).json({ model: result, message: 'Successfully fetch Employer 2FA Settings.', successful: true })
        } catch(err) {
            res.status(400).json({ message: err.message, successful: false })
        }
    })
